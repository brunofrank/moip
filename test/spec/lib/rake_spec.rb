...
describe PagSeguro::Rake do
  before(:each) do
    ...
    @origin = File.expand_path(File.dirname(__FILE__) + "/../fixtures/moip-test.yml")
    @destiny = RAILS_ROOT + "/tmp/moip-test.yml"
    ...
    # Load the moip-test.yml file to
  ...
  it "should be test environment" do
    MoipDeveloperController::MOIP_ORDERS_FILE.should == File.join(Rails.root, "tmp", "moip-test.yml")
  end
